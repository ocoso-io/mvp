...
function handleResize () {
  ...
  const wallet = document.querySelector('.wallet-button-wrapper');

  const phone       = isPhone();        
  const burgerMenu  = usesBurger();      


  if (hb)   hb.style.display   = burgerMenu ? 'flex' : 'none';
  if (main) main.style.display = burgerMenu ? 'none' : 'flex';


  if (wallet) wallet.style.display = phone ? 'none' : 'flex';*/
...
function loadOverlayMenu() {
  ...
  fetch('components/mobile-flag-nav3d-crossbrowser.html')
    ...
    .then(html => {
      ...
      /* <<<  NEU: jetzt ist der Burger im DOM → Navi neu anpassen  */
      handleResize();      
    })
