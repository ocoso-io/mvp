...
async function mintNFT(nftData) {
    // TODO: Implement your minting logic here
    // ...
    // ...
    // ...
    // ...
...
/**
 * Uploads the content of an NFT to IPFS.
 *
 * @param content - the content to upload.
 * @returns {Promise<string>} - Hash of content on IPFS.
 *
 * @example
 * const content = "Hello, world!";
 * const hash = await uploadNFTContent(content);
 * console.log(hash); // QmW2WQi7j6c7UgJTarActp7tDNikE4B2qXtFCfLPdsgaTQ
 *
 */
async function uploadNFTContent(content) {
    // is only sketch
...
(() => {
    ...
    const validateForm = () => {
        ...
        if (!selectedType) {
            alert('Bitte Inhaltstyp wählen.');
            return false;
        }
        if (!document.querySelector('.radio-btn.active')) {
            alert('Bitte Sichtbarkeit wählen.');
            return false;
        }
        return true;
    };
